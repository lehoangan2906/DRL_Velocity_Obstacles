...
ROOT = FILE.parents[1]  # YOLOv8 strongsort root directory
...
sys.path.append(str(FILE.parents[3]))  # add strong_sort ROOT to PATH
...
class TrackPedPublisher(Node):
    def __init__(self):
        ...
        self.declare_parameter("camera_source", "real")     # 'real' or 'simulated'
        ...
        self.stride = self.model.stride # Stride of the YOLOv8 model
        self.imgsz = check_imgsz((640, 640), stride=self.stride)  # Inference image size

        # Warmup the model (to redice initial inference latency)
        ...
        self.outputs = [None] * 1    # Previous frames for camera motion compensation
        ...
        # Handle input source (real camera or simulated camera)
        if self.camera_source == 'real':
            # RealSense configuration for depth and color streams if using the real camera
        ...
        elif self.camera_source == 'simulated':
            # Simulated camera case (subscribers for simulated camera topics)
            self.depth_image = None
            self.color_image = None
            self.depth_subscriber = self.create_subscription(Image, "/intel_realsense_d415_depth/depth/image_raw", self.depth_callback, 10)
            self.color_subscriber = self.create_subscription(Image, '/intel_realsense_d415_depth/image_raw', self.color_callback, 10)
            self.bridge = CvBridge()    # For converting ROS Image messages to OpenCV format

        # Calculate focal length in pixels based on the camera's field of vieew
        ...
        self.known_id = {}   # Dictionary of known person IDs
        ...
        # Set up dictionaries and save paths
    ...
    def timer_callback(self):
        if self.camera_source == 'real':
            ...
            if not depth_frame or not color_frame:
                self.get_logger().warn("RealSense camera frames are missing, skipping frame.")
                return   # Skip this frame if either depth or color frame is missing
            ...
            paths = ["webcam"]    # Using real camera (webcam)

        elif self.camera_source == 'simulated':
            # Simulated camera case
            if self.depth_image is None or self.color_image is None:
                self.get_logger().warn("Simulated camera images are missing, skipping frame.")
                return  # Skip this frame if images are missing

            im0s = [self.color_image]   # Use the color image from simulated camera
            depth_image = self.depth_image  # Use the depth image from simulated camera
            paths = ["simulated_camera"]

        else:
            # Use the source as the input path (for files or URLs)
            paths = [self.source]
            im0s = [cv2.imread(self.source)]   # Replace with actual image loading logic
            if im0s[0] is None:
                self.get_logger().error(f"Failed to load image from {self.source}, skipping frame.")
        ...
        for rs_dict in rs_dicts:
            ...
            if rs_dict["velocity"]:
                ...
                tracked_person_msg.twist.linear.z = 0.0
